...
class AzureContentUnderstandingClient:
    ...
    def begin_analyze(self, analyzer_id: str, file_location: str) -> Response:
        ...
        Routes to the correct GA API endpoint:
          - Local files → :analyzeBinary (application/octet-stream)
          - URLs → :analyze with {"inputs": [{"url": ...}]}
        ...
        # Determine if it's a file path or URL and route to correct GA method
        if file_path.exists() and file_path.is_file():
            return self.begin_analyze_binary(analyzer_id, file_location)
        elif file_location.startswith("https://") or file_location.startswith(
            "http://"
        ):
            return self.begin_analyze_url(analyzer_id, file_location)
        else:
            raise ValueError("File location must be a valid file path or URL.")
